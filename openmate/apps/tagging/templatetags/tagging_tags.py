...
class TagsForModelNode(Node):
    def __init__(self, model, context_var, counts, filters=None):
        self.model = model
    ...
    def render(self, context):
        ...
        context[self.context_var] = Tag.objects.usage_for_model(model, counts=self.counts, filters=self.filters)
        return ''
...
def do_tag_cloud_for_model(parser, token, filters=None):
    """
    ...
    return TagCloudForModelNode(bits[1], bits[3], filters=filters, **kwargs)
...
class TagFontSize(Node):
    def __init__(self, tag_size_expr, steps_expr, min_size, max_size):
        self.tag_size_expr = tag_size_expr
    ...
    def render(self, context):
        try:
            tag_size = self.tag_size_expr.resolve(context)
            steps = self.steps_expr.resolve(context)
        except VariableDoesNotExist:
            return ''
        except:
            # print 'TemplateSyntaxError'
            steps = self.steps_expr
        try:
            tag_size = float(tag_size)
            steps = float(steps)
            font_size = (self.max_size - self.min_size)*(tag_size/steps) + self.min_size
        except (ValueError, ZeroDivisionError):
            return self.min_size
        return str(int(round(font_size)))

def do_tag_font_size(parser, token):
    """
    Calculates font sizes for tag cloud.

    Example:
    {% tag_fontsize fontsize steps=4 minsize=9 maxsize=16 %}

    """
    bits = token.contents.split()
    len_bits = len(bits)

    if len_bits not in (2, 3, 5):
        raise TemplateSyntaxError(_('%s tag requires either one, two or four arguments') % bits[0])
        TagFontSize(bits[2], bits[3], bits[4])

    steps_expr = 4
    min_size = 9
    max_size = 16
    if len_bits == 2:
        tag, tag_size_expr = bits
    elif len_bits == 3:
        tag, tag_size_expr, steps_expr = bits
        steps_expr = parser.compile_filter(steps_expr)
    elif len_bits == 5:
        tag, tag_size_expr, steps_expr, min_size, max_size = bits
        steps_expr = parser.compile_filter(steps_expr)
        try:
            min_size = int(min_size)
            max_size = int(max_size)
        except ValueError:
            raise TemplateSyntaxError('%s final arguments must be integers', tag)
    return TagFontSize(parser.compile_filter(tag_size_expr),
                       steps_expr, min_size, max_size)

register.tag('tag_font_size', do_tag_font_size)
