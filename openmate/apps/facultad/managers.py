...
class AlumnoManager(models.Manager):
	def create(self, **kwargs):
		list = self.filter(user=kwargs['user'], carrera=kwargs['carrera'], plancarrera=kwargs['plancarrera'])
		if list.count() > 0:
			return None
	...
	def get_summary(self, user):
		list_carreras = self.select_related(depth=1).filter(user=user)
		for a in list_carreras:
			a.new_alumnos = self.filter(carrera=a.carrera).count()
			# m.new_messages = File.objects.filter(folder__group__email='fiuba-' + m.materia.id).count()
...
class AlumnoMateriaManager(models.Manager):
	...
	def get_initial_data(self, user, materia):
		...
		try:
			...
			if am.cursada_cuat:
				d['cursada_cuat'], d['cursada_year']  = am.cursada_cuat.split('-')
			if am.aprobada_cuat:
				d['aprobada_cuat'], d['aprobada_year']  = am.aprobada_cuat.split('-')
		except:
			pass
		return d
	...
	def list_materias_cursando(self, user):
		return self.list_materias(user=user).filter(Q(state='C') | Q(state='F'))

	def list_materias_aprobadas(self, user):
		return self.list_materias(user=user).filter(Q(state='A') | Q(state='E')).order_by('aprobada_date', 'materia')
	...
	def get_summary(self, user):
		...
		for m in list_matcur:
			#m.new_files = File.objects.filter(folder__group__email=m.materia.group_name()).count()
			#m.is_member = Member.objects.filter(group__email=m.materia.group_name(), user=user).count()
	...
	def create_or_update(self, user, materia, final_date, nota):
		try:
			...
			al = self.create(user=user, materia=materia, state='A', aprobada_date=final_date, nota=nota)
		...
		for al in list_carreras:
			...
			for m in materias_cursadas:
				...
				if materia.count():
					...
					# print '%s %s' % (materia[0].materia.codigo, materia[0].creditos)
					if m.nota > 0:
						al.promedio += m.nota
						count_materias += 1
			if count_materias:
...
class PlanMateriaManager(models.Manager):
	...
	def list_materias_para_cursar(self, user, plancarrera):
		...
		from facultad.models import AlumnoMateria, Correlativa, Alumno, PlanMateria
		...
		# Entonces... en co_list saco las que tienen alguna correlativa aprobada.
	...
	def list_materias_cursando(self, user, plancarrera):
		from facultad.models import AlumnoMateria, Correlativa
